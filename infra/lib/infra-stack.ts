...
export class InfraStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const vendorsTable = new dynamodb.Table(this, 'VendorsTable', {
      tableName: 'DerVendors',
      partitionKey: { name: 'phone', type: dynamodb.AttributeType.STRING },
      billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
      removalPolicy: cdk.RemovalPolicy.RETAIN, // Vendor applications must not be lost
    });

    // ── S3 Bucket: Vendor Uploads ────────────────────────────────────
    // Private bucket for vendor product images / licence documents,
    // accessed only via presigned URLs generated by the Next.js server.

    const vendorUploadsBucket = new s3.Bucket(this, 'VendorUploadsBucket', {
      bucketName: `der-vendor-uploads-${this.account}`,
      ...
      cors: [
        {
          ...
          allowedOrigins: [
            'https://der.bbin.in',
            'https://daawateramzaan.in',
            'https://www.daawateramzaan.in',
            'http://localhost:3000',
          ],
          allowedHeaders: ['*'],
        },
    ...
    new cdk.CfnOutput(this, 'VendorsTableName', {
      value: vendorsTable.tableName,
      description: 'DynamoDB table for vendor applications',
    });

    new cdk.CfnOutput(this, 'VendorUploadsBucketName', {
      value: vendorUploadsBucket.bucketName,
      description: 'S3 bucket for vendor uploads. The Next.js server IAM user needs dynamodb:* on DerVendors and s3:PutObject/GetObject on this bucket — grant manually since the app connects with static credentials, not a CDK-managed role.',
    });
  }
}
